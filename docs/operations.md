...
`granite-dev-graph-map` is centered on developer tools work in Rust. The technical target is: Build a Rust toolkit that studies graph behavior through transition tables, with invalid-transition tests and no network dependency.
